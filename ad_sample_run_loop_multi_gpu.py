...
parser.add_argument("--start", type=int, default=0, help="start of the pos")
parser.add_argument("--end", type=int, default=10, help="end of the pos")
parser.add_argument("--gpu_pos", type=int, default=0, help="which gpu to use")
...
if __name__ == '__main__':
    ...
    gpu_pos = opt.gpu_pos
    # os.environ["CUDA_VISIBLE_DEVICES"] = gpu_pos
    torch.cuda.set_device(gpu_pos)
    for j in range(start, end):
        run_main(j)
    pros_r = [i.cpu().numpy() for i in pros_results]
    pickle.dump(pros_r, open('pros_results_{}_{}.pkl'.format(start, end), 'wb'))
    # torch.nn.KLDivLoss
    # run_main(5)
    # run_main(5)